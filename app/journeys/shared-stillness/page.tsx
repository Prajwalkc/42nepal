...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const translation = getJourneyTranslation('shared-stillness');
  const title = translation?.title || "Shared Stillness";
  const description = translation?.description || "A journey for couples seeking presence, reconnection, and shared clarity through Himalayan stillness. Step away together to remember what matters.";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${title} - Couples retreat in the Himalayas`,
        },
      ],
    },
    twitter: {
...
export default async function SharedStillnessPage() {
  noStore();
  const { common, journeyDetail, getJourneyTranslation } = await getServerTranslations();
  const journey = getJourneyBySlug('shared-stillness');
  const translation = getJourneyTranslation('shared-stillness');
  
  const title = translation?.title || "Shared Stillness";
  const subtitle = translation?.subtitle || "For couples seeking presence and reconnection";
