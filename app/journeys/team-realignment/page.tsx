...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const translation = getJourneyTranslation('team-realignment');
  const title = translation?.title || "Collective Ascent";
  const description = translation?.description || "Shared elevation creates shared understanding. This journey brings teams into a quiet, neutral environment where alignment, trust, and perspective return.";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${title} - Team retreat in the Himalayas`,
        },
      ],
    },
    twitter: {
...
export default async function TeamRealignmentPage() {
  noStore();
  const { common, journeyDetail, getJourneyTranslation } = await getServerTranslations();
  const journey = getJourneyBySlug('team-realignment');
  const translation = getJourneyTranslation('team-realignment');
  
  const title = translation?.title || "Collective Ascent";
  const subtitle = translation?.subtitle || "For small teams (4–12) and leadership groups";
    ...
    <div className="min-h-screen bg-background text-[#2d3a47] scroll-smooth">
      ...
      <section className="relative py-40 px-6 sm:px-12 lg:px-24 bg-white pt-32 overflow-hidden">
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-6">
          ...
          <p className="text-xl sm:text-2xl text-[#3d5a7a] font-normal">
            {subtitle}
          </p>
          {description && (
          ...
          <div className="flex items-center justify-center gap-4 pt-2">
            ...
            <span className="text-small text-[#6b7786]">{journey?.location || "Annapurna Region"}</span>
          </div>
